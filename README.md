...
4. Once done swap the raw ip in `inventory.yml` with the new tailscale magicDNS name, and add the sudo password to `secrets.yml`. Add the magicDNS name (and user) to the ssh config as well.
5. Run `make post-install` on the fresh machine.
